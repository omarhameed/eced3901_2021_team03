...
UPDATED by Usman Kamran (Group 3) with functional LED.
...
ros::Subscriber<std_msgs::Float64> ledsub("led_input", &messageLedCb ); //LED subscriber
...
//Publisher led
std_msgs::Float64 led_msg;
ros::Publisher ledpub("led_listen", &led_msg); //led on or off
...
void setup()
{
        ...
        //led subscriber and publisher advertised and subscribed
        nh.subscribe(ledsub);
        nh.advertise(ledpub); 
...
void loop()
{
        ...
        //led 
        led_msg.data = led_data; 
        ledpub.publish(&led_msg);      
        nh.spinOnce();
        delay(1);
}
